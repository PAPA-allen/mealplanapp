...
const HomeScreen = () => {
  ...
  const deleteItemsByDate = async () => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:8000/deleteItems/${dateToDelete}`,
      );
  ...
  const copyItems = async () => {
    ...
    const response = await axios.post('http://localhost:8000/copyItems', {
  ...
  const renderWeekDates = startOfWeek => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      weekDates.push(
        <View   key={i} style={{flexDirection: 'row', gap: 12, marginVertical: 10}}>
          <View
            style={[
              {
                height: 40,
          ...
          <Pressable
            ...
            <Pressable
              ...
              style={{position: 'absolute', bottom: 5, right: 50}}>
